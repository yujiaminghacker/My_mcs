...
<div class='warp'>
    ...
    <table class="table2">
        ...
        <list from="$data" name="c">
            <tr>
                <td>{$c.id}</td>
                <td>{$c.title} <if value="$c.flag">[{$c.flag}]</if></td>
                <td>{$c.cat_name}</td>
                <td>{$c.addtime|date:'Y-m-d H:i',@@}</td>
                <td>{$c.click}</td>
                <td>
                    <a href='{|U:"del",array("id"=>$c["id"])}' onclick='return confirm("确定要删除吗？")'>删除</a> |
                    <a href="{|U:'edit',array("id"=>$c["id"])}">编辑</a> |
                    <a href="{|U:'Index/index/content',array("id"=>$c["id"])}" target="_blank">查看</a>
                </td>
            </tr>
        </list>    
    </table>
